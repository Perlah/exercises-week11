...
var mySketch = new p5(function(sketch){
  ...
  function getLocation() {
    currentLocation = prompt("Where are you?");
    loadData(currentLocation);
  }

  function resetSketch(newData) {
    data = newData;
    rectWidth = 0;
  }

  function loadData(where) {
    var url = "http://api.openweathermap.org/data/2.5/weather?q="+where;
    sketch.loadJSON(url, resetSketch);
  }
  ...
  sketch.draw = function() {
    ...
    if(data) {
      ...
      sketch.textSize(50);
      sketch.text(currentLocation + " ("+data.weather[0].description+")", 50, sketch.windowHeight/2);
    }
  };
